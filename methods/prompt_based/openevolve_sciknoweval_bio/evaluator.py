"""OpenEvolve evaluator for sciknoweval-bio prompt optimization."""
...
from cl.evals.sciknoweval_bio import _extract_mcq_answer, load_sciknoweval_bio_raw
from cl.utils.token_tracker import record_usage_to_file

CONFIG_PATH = os.environ.get("BENCHMARK_CONFIG", "configs/openevolve_sciknoweval_bio.yaml")
...
train_set = load_sciknoweval_bio_raw(
...
def _score_one(prompt_template: str, ex: dict) -> float:
    ...
    pred = _extract_mcq_answer(_extract_answer(response))
    ...
    def _on_complete(score):
        ...
        with lock:
            ...
            if done % 50 == 0 or done == n:
                ...
                print(f"[evaluator] sciknoweval-bio: {done}/{n} examples, running acc={avg:.3f}")
