"""OpenEvolve evaluator for HoVer prompt optimization.
...
from cl.evals.hover import load_hover_raw
from cl.utils.token_tracker import record_usage_to_file

# --- Load configuration from YAML ---
CONFIG_PATH = os.environ.get("BENCHMARK_CONFIG", "configs/openevolve_hover.yaml")
...
def _extract_label(response: str) -> str:
    """Extract the label from the model response.

    Looks for text after "Label:" (case-insensitive). If not found, searches
    for SUPPORTED/NOT_SUPPORTED anywhere in the response.
    """
...
def _score_one(prompt_template: str, ex: dict) -> float:
    """Score a single example. Returns 1.0 if correct, 0.0 otherwise."""
    try:
        formatted = prompt_template.format(claim=ex["claim"])
    except (KeyError, ValueError, IndexError):
        return 0.0
    response = _call_llm(formatted)
    pred = _extract_label(response)
    return 1.0 if pred == ex["label"] else 0.0


def _score_prompt(prompt_template: str, examples: list[dict]) -> float:
    """Evaluate a prompt template on a list of examples in parallel, return mean accuracy."""
    n = len(examples)
    if n == 0:
        return 0.0
    ...
    def _on_complete(score):
        ...
        with lock:
            ...
            if done % 50 == 0 or done == n:
                avg = total / done
                print(f"[evaluator] hover: {done}/{n} examples, running acc={avg:.3f}")
...
def _validate_prompt_structure(prompt_template: str) -> None:
    ...
    if "{claim}" not in prompt_template:
        raise ValueError("prompt missing required {claim} placeholder")
    try:
        prompt_template.format(claim="__probe__")
