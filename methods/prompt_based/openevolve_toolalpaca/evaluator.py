"""OpenEvolve evaluator for ToolAlpaca prompt optimization.
...
from cl.evals.toolalpaca import load_toolalpaca_raw, _parse_actions, _score_actions
from cl.utils.token_tracker import record_usage_to_file

# --- Load configuration from YAML ---
CONFIG_PATH = os.environ.get("BENCHMARK_CONFIG", "configs/openevolve_toolalpaca.yaml")
...
train_set, _ = load_toolalpaca_raw(
...
def _score_one(prompt_template: str, ex: dict) -> float:
    """Score a single example. Returns score 0-1."""
    try:
...
def _score_prompt(prompt_template: str, examples: list[dict]) -> float:
    """Evaluate a prompt template on examples in parallel, return mean score."""
    n = len(examples)
    if n == 0:
        return 0.0
    ...
    def _on_complete(score):
        ...
        with lock:
            ...
            if done % 20 == 0 or done == n:
                ...
                print(f"[evaluator] toolalpaca: {done}/{n} examples, running score={avg:.3f}")
