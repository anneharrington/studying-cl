"""OpenEvolve evaluator for FinQA prompt optimization.
...
from cl.evals.finqa import _extract_predicted_number, _numbers_match, load_finqa_raw
from cl.utils.token_tracker import record_usage_to_file

CONFIG_PATH = os.environ.get("BENCHMARK_CONFIG", "configs/openevolve_finqa.yaml")
...
def _score_one(prompt_template: str, ex: dict) -> float:
    ...
    return 1.0 if _numbers_match(pred, ex["answer"]) else 0.0
    ...
    def _on_complete(score):
        ...
        with lock:
            ...
            if done % 50 == 0 or done == n:
                ...
                print(f"[evaluator] finqa: {done}/{n} examples, running acc={avg:.3f}")
