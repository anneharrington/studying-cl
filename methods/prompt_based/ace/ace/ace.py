...
class ACE:
    ...
    def __init__(
        ...
        reflector_api_provider: str | None = None,
        reflector_max_tokens: int | None = None,
    ):
        """
        ...
        Args:
            ...
            enable_thinking: If False, propagates to Generator/Reflector/Curator
                so they inject provider-specific extra_body disabling qwen3
                reasoning mode on every LLM call.
            reflector_api_provider: If set, the api_provider the Reflector
                reports to its LLM calls — use it when the reflector client
                (see initialize_clients) is on a different provider than the
                generator/curator. Defaults to api_provider (single-provider).
            reflector_max_tokens: If set, the max_tokens budget for the
                Reflector only. Defaults to max_tokens. Useful when the
                reflector is a reasoning model (e.g. GPT-5.2) that needs more
                headroom than the task LM's provider cap allows.
        """
        # Initialize API clients
        generator_client, reflector_client, curator_client = initialize_clients(api_provider)

        # Reflector may run on a different provider than generator/curator;
        # fall back to the shared api_provider / max_tokens when not overridden.
        reflector_provider = reflector_api_provider or api_provider
        reflector_tokens = reflector_max_tokens or max_tokens

        # Initialize the three agents
        self.generator = Generator(generator_client, api_provider, generator_model, max_tokens,
                                   enable_thinking=enable_thinking, extra_body=extra_body)
        self.reflector = Reflector(reflector_client, reflector_provider, reflector_model, reflector_tokens,
                                   enable_thinking=enable_thinking, extra_body=extra_body)
...
## OTHERS"""
    ...
    def _extract_config_params(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            # Optional early stopping on val plateau. None = disabled (default).
            # When set to N, training stops after N consecutive val checkpoints
            # with no improvement over best_accuracy.
    ...
    def _offline_train(
        ...
        # Optional early stopping on val plateau (None = disabled).
        early_stopping_patience = config_params.get('early_stopping_patience')
        ...
        if early_stopping_patience is not None:
            print(f"Early stopping patience: {early_stopping_patience} val checkpoints")
        print()
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step, task_dict in enumerate(train_samples):
                ...
                if step % eval_steps == 0:
                    ...
                    # Track best playbook (and optionally early-stop on plateau)
                    if val_results:
                        ...
                        if acc > best_accuracy:
                            ...
                            if (early_stopping_patience is not None
                                ...
                                print(f"⏹️  Early stopping: val didn't improve for "
                                      f"{early_stopping_patience} consecutive checkpoint(s); "
                    ...
                    if should_stop:
                        break  # exit the step loop; outer epoch loop will also break
