...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # VeOmni's plaintext-iterable dataloader emits 1D packed (N,) input_ids
        # giving 2D inputs_embeds (N, hidden). transpose(1,2) inside attention
        # then breaks (q ends up (N, head_dim, num_heads)). Add a batch dim so
        # the 4D attention layout works. Idempotent: only fires on 2D input.
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            # VeOmni's packed-plaintext dataloader passes a 1D `attention_mask`
            # (cu_seqlens-like). transformers' create_causal_mask -> per-impl
            # mask functions assume 2D `(batch, seq)` and IndexError on 1D.
            # For packed sequences we don't need a 2D padding mask anyway —
            # is_causal=True (SDPA) or cu_seqlens (FA-varlen) handles the
            # boundaries. Drop 1D masks here so create_causal_mask returns
            # None and the model falls back to is_causal.
            mask_for_create = attention_mask
            if isinstance(attention_mask, torch.Tensor) and attention_mask.dim() == 1:
                mask_for_create = None
            # Prepare mask arguments
            mask_kwargs = {
                "config": self.config,
                "input_embeds": inputs_embeds,
                "attention_mask": mask_for_create,
                "cache_position": cache_position,
                "past_key_values": past_key_values,
                "position_ids": position_ids,
            }
