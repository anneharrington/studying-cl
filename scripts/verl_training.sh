#!/bin/bash
# The verl package is vendored under <repo>/engine/verl. Ensure it is importable
# even when this driver is invoked directly (run.sh also sets this).
_VERL_SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
export PYTHONPATH="${_VERL_SCRIPT_DIR}/../engine:${PYTHONPATH:-}"
