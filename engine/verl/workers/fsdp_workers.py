...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        # Resolve through the FSDP wrapper: FSDP exposes the wrapped PEFT model
        # via `_fsdp_wrapped_module`. The original check did `hasattr(...)` on
        # the FSDP wrapper itself, which doesn't surface `peft_config`, so on
        # FSDP-wrapped PEFT models this branch silently no-opped and no adapter
        # was ever saved. Mirror the access pattern used elsewhere in this file
        # (line 688) so the LoRA save fires for FSDP+PEFT.
        peft_model = getattr(self.actor_module_fsdp, "_fsdp_wrapped_module", self.actor_module_fsdp)
