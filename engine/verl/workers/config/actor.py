...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        demo_source (str): Source of the demonstration used to build the teacher's ICL prompt.
            "self_success" (default, SDPO): pick a successful rollout from the current group;
            "gold": use the gold demonstration attached to each prompt under
            `batch.non_tensor_batch["gold_demo"]` (required for SDFT, Shenfeld et al. arXiv:2601.19897).
        num_loss_tokens_to_skip (int): Zero out the first N response-mask positions per row before
            computing distillation loss. Matches SDFT's `num_loss_tokens_to_skip=3`
            (idanshen/Self-Distillation/main.py). 0 disables (SDPO default).
    """
    ...
    def __post_init__(self):
        ...
        # "gold": teacher distribution is one-hot on the response tokens (which must be the
        # dataset's gold answer — see actor_rollout_ref.rollout.use_gold_response). Combined
        # with alpha=0 forward-KL, this reduces SDPO to plain SFT (NLL on gold tokens). See
        # core_algos.compute_self_distillation_loss for the math.
        valid_teacher_regularization = ["ema", "trust-region", "gold"]
        ...
        valid_demo_sources = ["self_success", "gold"]
        if self.demo_source not in valid_demo_sources:
            raise ValueError(
                f"self_distillation.demo_source must be one of {valid_demo_sources}, got {self.demo_source}"
            )
        if self.num_loss_tokens_to_skip < 0:
            raise ValueError(
                f"self_distillation.num_loss_tokens_to_skip must be >= 0, got {self.num_loss_tokens_to_skip}"
            )
