...
try:
    # vllm >= 0.12 exposes CoreEngineProcManager from this path. Older releases
    # (e.g. 0.8.4 which the SDPO README recommends for H100/GH200) don't ship it;
    # it is only needed by run_headless() for multi-node rollout workers, which
    # we never hit on single-node runs.
...
if _VLLM_VERSION > version.parse("0.11.0"):
    ...
    try:
        # Added to vllm.utils in ~0.9; absent in 0.8.4 which the SDPO README pins.
        # Only referenced by run_headless() (multi-node rollout), which also requires
        # CoreEngineProcManager and is already gated below, so None is safe here.
...
class vLLMHttpServer:
    ...
    async def run_headless(self, args: argparse.Namespace):
        ...
        if get_tcp_uri is None or CoreEngineProcManager is None:
            raise RuntimeError(
                "run_headless() requires vllm>=0.12 (get_tcp_uri / CoreEngineProcManager "
                "are only available there). The installed vllm does not ship them. This "
                "path only fires for multi-node rollouts; use a single-node setup or upgrade vllm."
            )
