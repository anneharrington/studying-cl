        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self_distillation_enabled:
                        if teacher_regularization == "gold":
                            # Plain-SFT reduction: teacher is a one-hot distribution on the
                            # response tokens (which the trainer has substituted with the
                            # dataset's gold via actor_rollout_ref.rollout.use_gold_response).
                            # F.kl_div with alpha=0 then collapses to -log p_S(gold) = NLL.
                            # No teacher forward pass needed; no EMA/trust-region module.
                            response_ids = model_inputs["responses"]              # (bsz, resp_len)
                            teacher_log_prob = torch.zeros_like(log_prob)         # log 1 = 0 at gold
                            if return_all_logps:
                                if student_all_logps is None:
                                    raise ValueError(
                                        "teacher_regularization='gold' with full_logit_distillation "
                                        "requires student_all_logps."
                                    )
                                NEG = -1e9
                                teacher_all_logps = torch.full(
                                    (response_ids.size(0), response_ids.size(1), student_all_logps.size(-1)),
                                    NEG, dtype=student_all_logps.dtype, device=student_all_logps.device,
                                )
                                teacher_all_logps.scatter_(2, response_ids.unsqueeze(-1).long(), 0.0)
                            else:
                                teacher_all_logps = None
                            teacher_topk_logps = None  # gold mode is full-logit only
                        else:
                            teacher_inputs = {
