...
def compute_self_distillation_loss(
    ...
    num_loss_tokens_to_skip = getattr(self_distillation_config, "num_loss_tokens_to_skip", 0) or 0
    if num_loss_tokens_to_skip > 0:
        # SDFT (Shenfeld et al. arXiv:2601.19897): exclude the first N response tokens from
        # the distillation loss; mirrors num_loss_tokens_to_skip in idanshen/Self-Distillation.
        seq_len = loss_mask.size(1)
        positions = torch.arange(seq_len, device=loss_mask.device)
        skip_mask = (positions >= num_loss_tokens_to_skip).to(loss_mask.dtype)
        loss_mask = loss_mask * skip_mask
