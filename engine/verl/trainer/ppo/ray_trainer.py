...
class RayPPOTrainer:
    ...
    def _maybe_build_self_distillation_batch(
        ...
        demo_source = self_distillation_cfg.get("demo_source", "self_success")
        if demo_source == "gold":
            # SDFT (Shenfeld et al. arXiv:2601.19897): teacher demonstration comes from the
            # dataset's gold answer, not from self-successes. Every prompt is distilled every step.
            # Reuse verl's canonical ground-truth field (reward_model.ground_truth) so no dataset
            # loader edits are required.
            if "reward_model" not in batch.non_tensor_batch:
                raise KeyError(
                    "self_distillation.demo_source='gold' requires reward_model.ground_truth "
                    "on each sample (verl's standard preprocessing convention)."
                )
            reward_models = batch.non_tensor_batch["reward_model"]
            solution_strs = [
                str(reward_models[i].get("ground_truth")) if reward_models[i].get("ground_truth") is not None else None
    ...
    def _substitute_gold_responses(self, batch: DataProto, gen_batch_output: DataProto) -> None:
        """In-place replace the rollout-generated `responses` (and the response portion of
        `sequences`/`attention_mask`) with the dataset's gold answer.

        Used by the plain-SFT reduction (rollout.use_gold_response=true). The rollout still
        runs (so vLLM is initialized for val), but its output is overwritten before any loss
        computation. Combined with self_distillation.alpha=0 + teacher_regularization=gold +
        is_clip=null, the SDPO loss reduces to NLL on gold tokens. eos_token_id is appended
        as the response terminator (for Qwen-style chat templates this is `<|im_end|>`).

        Tooluse-format adaptation: the tooluse dataset stores ground_truth as a JSON-serialized
        list of {Action, Action_Input} dicts (the form the scorer parses *into*), but the
        agent_loop scorer expects model output in ReAct text form ("Action: X\\nAction Input: Y").
        Tokenizing the JSON directly would train the model to emit a string the scorer cannot
        read → score=0 even though NLL is decreasing. We sniff for that JSON shape and convert
        it to ReAct text first; bio (e.g. "B") and finqa (e.g. "829") gold strings are not lists
        of action dicts so they fall through unchanged. The ReAct form round-trips through
        the scorer's parser back to score=1.0.
        """
        import json

        def _gold_to_training_target(gt) -> str:
            """If gt looks like tooluse JSON list, emit ReAct text; else stringify as-is."""
            if not isinstance(gt, str):
                return str(gt)
            stripped = gt.lstrip()
            if not stripped.startswith("["):
                return gt  # bio / finqa: plain answer string, leave alone
            try:
            ...
            if not isinstance(parsed, list) or not parsed:
                return gt
            if not all(isinstance(a, dict) and "Action" in a and "Action_Input" in a for a in parsed):
                return gt
            return "\n".join(f"Action: {a['Action']}\nAction Input: {a['Action_Input']}" for a in parsed)

        tokenizer = self.tokenizer
        eos_id = tokenizer.eos_token_id
        if eos_id is None:
            raise ValueError("use_gold_response requires the tokenizer to define eos_token_id.")
        pad_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else eos_id

        responses = gen_batch_output.batch["responses"]                # (bsz, resp_len)
        bsz, resp_len = responses.shape
        device = responses.device

        gt_field = batch.non_tensor_batch.get("reward_model")
        if gt_field is None:
            raise KeyError("use_gold_response requires reward_model.ground_truth on each sample.")

        new_responses = torch.full((bsz, resp_len), pad_id, dtype=responses.dtype, device=device)
        new_resp_attn = torch.zeros((bsz, resp_len), dtype=torch.long, device=device)
        for i in range(bsz):
            gt = gt_field[i].get("ground_truth") if gt_field[i] is not None else None
            if gt is None:
                continue
            target_text = _gold_to_training_target(gt)
            ids = tokenizer.encode(target_text, add_special_tokens=False)
            ids = ids + [eos_id]
            if len(ids) > resp_len:
                ids = ids[:resp_len]
            new_responses[i, : len(ids)] = torch.tensor(ids, dtype=responses.dtype, device=device)
            new_resp_attn[i, : len(ids)] = 1

        gen_batch_output.batch["responses"] = new_responses

        # `sequences` is prompts ⊕ responses; only the trailing response_length cols change.
        if "sequences" in gen_batch_output.batch.keys():
            seq = gen_batch_output.batch["sequences"]
            seq[:, -resp_len:] = new_responses
            gen_batch_output.batch["sequences"] = seq

        # `attention_mask` is full-length; update only the response portion.
        if "attention_mask" in gen_batch_output.batch.keys():
            attn = gen_batch_output.batch["attention_mask"]
            attn[:, -resp_len:] = new_resp_attn.to(attn.dtype)
            gen_batch_output.batch["attention_mask"] = attn

        # Old logp came from the rollout policy; with substituted responses it's stale.
        # We zero it out — the SFT config sets is_clip=null so the IS-ratio path is skipped.
        if "old_log_probs" in gen_batch_output.batch.keys():
            gen_batch_output.batch["old_log_probs"] = torch.zeros_like(
                gen_batch_output.batch["old_log_probs"]
            )
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # Plain-SFT reduction: replace rollout responses with the dataset's gold
                        # answer. Combined with self_distillation.alpha=0 + teacher_regularization=
                        # gold + is_clip=null, the SDPO loss collapses to NLL on gold tokens (= SFT).
                        if self.config.actor_rollout_ref.rollout.get("use_gold_response", False):
                            self._substitute_gold_responses(batch, gen_batch_output)
